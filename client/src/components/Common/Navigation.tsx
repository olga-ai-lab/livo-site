...
export const Navigation: React.FC<NavigationProps> = ({ type }) => {
  ...
  const brandColor = isLivo ? 'text-[#7AB72D]' : 'text-[#353E4A]';
  const hoverColor = isLivo ? 'hover:text-[#7AB72D]' : 'hover:text-[#353E4A]';
  const buttonVariant = isLivo ? 'lime' : 'default';
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-b border-border h-20 shadow-sm">
      <div className="container h-full flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          <Link
            href="/blog"
            className={cn("text-sm font-medium transition-colors uppercase tracking-wide", location === '/blog' ? brandColor + " font-bold" : "text-foreground/60", hoverColor)}
          >
            Blog
          </Link>
          {type !== 'blog' && (
            <Link
              href="/sinistro-novo"
              className={cn("text-sm font-medium transition-colors uppercase tracking-wide", location === '/sinistro-novo' ? brandColor + " font-bold" : "text-foreground/60", hoverColor)}
            >
              Sinistro
            </Link>
          )}
